  ...
  return (
    <div className="fixed inset-0 min-h-screen min-w-full flex items-center justify-center overflow-hidden bg-black">
      ...
      >
        ...
        <div className="mb-8 w-full px-0" style={{maxWidth: '100vw'}}>
          <input
            type="text"
            placeholder="Search videos..."
            value={search}
            onChange={e => setSearch(e.target.value)}
            className="w-full px-5 py-3 rounded-xl border border-gray-300 focus:border-blue-400 focus:ring-2 focus:ring-blue-100 text-lg bg-white/70 shadow-sm placeholder:text-gray-400"
            style={{width: '100vw', maxWidth: '100vw'}}
          />
        </div>
        <div className="flex flex-col items-center justify-center min-h-[320px] w-full px-8 pb-8">
          {videos.length === 0 ? (
              ...
              >
                ...
                {videos.filter(v => v.watched).map(video => (
                  <Link
                    key={video.videoId}
                    to={`/video/${video.videoId}`}
                    className="group flex flex-col items-center min-w-[220px] max-w-[220px] bg-white/60 rounded-2xl shadow-lg hover:shadow-2xl transition-shadow duration-200 border border-gray-200 hover:border-blue-400 cursor-pointer"
                    style={{ textDecoration: 'none' }}
                  >
                    <video
                      src={`http://localhost:8080/api/v1/videos/${video.videoId}/file`}
                      className="w-full h-40 object-cover rounded-t-2xl bg-gray-200 group-hover:scale-105 transition-transform duration-200"
                      controls={false}
                      muted
                      preload="metadata"
                      onMouseOver={e => e.currentTarget.play()}
  ...
  if (error) return (
    <div className="spline-bg min-h-screen flex items-center justify-center">
      <div className="glass-card max-w-2xl w-full mx-auto p-8">
        ...
        <Link to="/" className="upload-btn mt-4 inline-block">Go Home</Link>
      </div>
    </div>
  );
  if (!video) return (
  ...
  return (
    <div className="spline-bg min-h-screen flex items-center justify-center">
      <div className="glass-card max-w-2xl w-full mx-auto p-8">
        ...
        <div className="mb-4 text-gray-700">{video.description}</div>
        <VideoPlayer src={`http://localhost:8080/api/v1/videos/${video.videoId}/file`} />
      </div>
    </div>
  );
}
